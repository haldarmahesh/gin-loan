import 'package:contra_loan_app/core/network/mock_http_client.dart';
import 'package:contra_loan_app/data/models/loan_model.dart';
import 'package:flutter/material.dart';

class LoanRemoteDatasource {
  // TODO(mahesh): replace with httpClient

  final MockHttpClient<LoanModel> mockHttpClient;
  LoanRemoteDatasource({@required this.mockHttpClient});

  Future<LoanModel> getLoanDetail() {
    var data = {
      "loan1": {
        "type": "loan_type",
        "title": "Bimala Devi’s Shed Construction Loan",
        "image": {
          "url":
              "https://lh3.googleusercontent.com/4jdIr7IAp3DVx_Ss_JKAc4aKuWo5KRMUNGkhP-J60kvQ6R-zR1Tt9LbPhVASEO3iasx2X9SgmGZzmk1SDPE7vSz9hMLT",
          "label": "Bimala Devi (Female,47)"
        },
        "Borrower Location": {
          "lat": 29.9230305,
          "lng": 78.1287824,
          "address":
              "105/9 Arihant Vihar, Near Uttarakhand Public Service Commission, Haridwar, Uttarakhand"
        },
        "Applicant Details": {
          "Name": "Bimala Devi",
          "Date of Birth": "10th May 1959",
          "Phone Number": ["9999987654"],
          "Marital Status": "Married",
          "No of Dependents": 3
        },
        "Loan Terms": {
          "Duration": "2 years",
          "Interest Rate": "12% per annum",
          "Loan Amount": 25000,
          "Loan Product": "Shed Construction"
        },
        "Repayment Schedule": [
          {"Date": "2nd Jan 2019", "Amount": 500},
          {"Date": "2nd Dec 2018", "Amount": 505},
          {"Date": "2nd Nov 2018", "Amount": 509},
          {"Date": "2nd Oct 2018", "Amount": 514},
          {"Date": "2nd Sept 2018", "Amount": 518}
        ]
      }
    };
    LoanModel loanData = new LoanModel.fromJson(data);
    return this.mockHttpClient.getData(loanData);
  }
}
